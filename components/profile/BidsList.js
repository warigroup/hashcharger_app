...
class BidsList extends React.Component {
	render() {
		...
		const bidsList = this.props.bids.map(function(bid, index) {
			return (
				<tr
				...
				>
					...
					<td className="orders-table-duration">
						<p className="tabledata">
							{convertDuration(bid.duration)}
						</p>
					</td>

					<td className="orders-table-average-hashrate">
						<p className="tabledata">
							{bid.reserved_hashrate === null ? '0.0000' : bid.reserved_hashrate} /{' '}
							{bid.average_hashrate === null ? '0.0000' : bid.average_hashrate} {bid.hashrate_units}H/s
						</p>
					</td>
					...
					<td className="orders-table-status">
						<p className="tabledata">
							...
							{bid.payment_success === false && bid.payment_fail === false ? 'Waiting for Payment' : ''}
						</p>
					</td>
					...
					<td className="orders-table-mining">
						<p className="tabledata">
							{/***** WAITING FOR PAYMENT  ****/}
							{bid.payment_success === false && bid.payment_fail === false ? 'N/A' : null}
							{/***** PAYMENT FAILED  ****/}
							{bid.payment_success === false && bid.payment_fail === true ? 'N/A' : null}
							{/***** PAID, MINING STARTED  ****/}
							{bid.payment_success === true &&
							bid.settlement_start_time !== null &&
							bid.settlement_is_finished === false
								? 'In Progress'
								: null}
					...
					<td className="orders-table-buttons text-xl-center text-lg-center text-md-right text-right">
						<button
